...
- **Dev mode is a different loop and needs no asar repack.** With `bun run dev:desktop`, the GUI serves from `packages/desktop/out/` and bundles `packages/opencode/dist/node/node.js` into `out/main`. Renderer changes (`packages/app`, `packages/ui`) hot-reload live through Vite; `packages/desktop/src` main/preload changes trigger an automatic electron-vite rebuild and app restart. The server bundle is **not** watched: to apply `packages/opencode` source changes, run `bun run dev:desktop:restart` from the repo root, which rebuilds `dist/node/node.js` (via `predev`) and relaunches the GUI.
